...
inline fun <E> Collection<E>.iterateLast(element: (E) -> Unit, last: (E) -> Unit) = iterateLast(iterator(), element, last)

inline fun <E> sepBy(i: Iterator<E>, string: StringBuilder, sep: String, f: (E) -> Unit) {
    iterateLast(i, {f(it); string.append(sep)}, {f(it)})
}

inline fun <E> Collection<E>.sepBy(string: StringBuilder, sep: String, f: (E) -> Unit) = sepBy(iterator(), string, sep, f)